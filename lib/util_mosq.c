...
#endif
{
	...
	if(mosq->bridge && mosq->bridge->start_type == bst_lazy && mosq->sock != INVALID_SOCKET) {
		int idle_time = now - (mosq->next_msg_out - mosq->keepalive);
		//_mosquitto_log_printf(NULL, MOSQ_LOG_NOTICE, "Checking lazy bridge %s idle timeout now:%d next_msg_out:%d keepalive:%d idle_timeout:%d idle_time:%d", mosq->id, now, mosq->next_msg_out, mosq->keepalive, mosq->bridge->idle_timeout, idle_time);
		if (idle_time >= mosq->bridge->idle_timeout){
...
#if defined(WITH_BROKER) && defined(WITH_BRIDGE)
			(!mosq->bridge || mosq->bridge->start_type != bst_lazy) && // don't keep-alive this for lazy bridges
#endif
